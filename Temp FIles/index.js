...
const controller =
  process.env.NODE_ENV === "test"
    ? require("../../diabetesmatecontrollers/opt/appointments.controller")
    : require("/opt/appointments.controller.js");
const authoriser =
  process.env.NODE_ENV === "test"
    ? require("../../diabetesmateauthorisers/opt/main.authorisers")
    : require("/opt/main.authorisers.js");
...
exports.handler = async (event, context) => {
  ...
  switch (event?.["pathParameters"]?.["action"]?.toLowerCase()) {
    ...
    case "delete":
      ...
      return await controller.deleteAppointmentsForUser(
        JSON.parse(event.body),
        authUserId
      );
    case "test-auth":
