const queries =
  process.env.NODE_ENV === "test"
    ? require("../../diabetesmatequeries/opt/appointments.queries")
    : require("/opt/appointments.queries.js");
const { validateBiomarker } =
  process.env.NODE_ENV === "test"
    ? require("../../diabetesmatevalidators/opt/appointments.validators")
    : require("/opt/appointments.validators.js");
...
module.exports = {
  ...
  async deleteAppointmentsForUser(appointment, authUserId) {
    return await queries.deleteAppointmentsForUser(appointment, authUserId);
  },
  /**
  ...
  async updateAppointment(appointment, authUserId) {
    ...
    let validationErrorArray = validateBiomarker(
      appointment["biomarker"]["data"]
    );
