...
def do_push(state_token, factor_id):
    ...
    while r["status"] == "MFA_CHALLENGE" and r["factorResult"] == "WAITING" and attempt < 30:
        ...
        try:
           if "correctAnswer" in r["_embedded"]["factors"]["_embedded"]["challenge"]:
               print("Your MFA 3 Number Challenge Answer is:",r["_embedded"]["factors"]["_embedded"]["challenge"]["correctAnswer"])
...
def auth_role(role, assertion, config_path = None):
    ...
    if not credentials.has_section(section_name):
        logger.debug("Creating new section: %s" % section_name)
        credentials.add_section(section_name)

    credentials.set(section_name, 'output', 'json')
    credentials.set(section_name, 'region', 'ap-southeast-2')
    credentials.set(section_name, 'aws_access_key_id', token["Credentials"]["AccessKeyId"])
    credentials.set(section_name, 'aws_secret_access_key', token["Credentials"]["SecretAccessKey"])
    credentials.set(section_name, 'aws_session_token', token["Credentials"]["SessionToken"])

    credential_path = os.path.abspath(os.path.expanduser("~/.aws/credentials"))

    with open(credential_path, 'w+') as credentialfile:
        credentials.write(credentialfile)
    
    #
    # --- NEW END: Write the credential file ---
    # 
